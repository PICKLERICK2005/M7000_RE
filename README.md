...
The next milestone is read-only characterization of the stock administration interface and the network activity generated by its frontend. USB enumeration follows that software-facing capture. Active probing, soldering, shield removal, and firmware writes are intentionally deferred until the relevant interfaces and recovery options are understood.

Detailed findings are recorded in [Recon 001](docs/recon-001.md). Sanitized templates for the current experiment are under [`captures/http/soft-capture-001/`](captures/http/soft-capture-001/).
