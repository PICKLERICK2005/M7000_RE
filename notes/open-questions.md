...
- Where is the `<stat>` field of the `+COPS=?` list response (unknown / available / current / forbidden) produced? It is on a different code path from the current-operator formatter decoded so far.
