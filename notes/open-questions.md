...
- How are the remaining bytes of the 80-byte `GET_NETWORK_OPERATOR_INFO_CNF` partitioned? The status word, the `<= 3` state field, the identity block at `+0x04`, flags at `+0x29`/`+0x2a` and two halfwords at `+0x48`/`+0x4a` are located; the long/short name extents and any access-technology field are not.
